    ...
    <div className="agente-servico-bg">
      <form className="form-card" onSubmit={handleSubmit}>
        ...
        <div className="form-title">
          <FaQuestionCircle style={{ color: "#3b70d1" }} /> Solicitar instrução de equipamento
        </div>
        <div className="form-group">
          <label>
            Equipamento 
            <textarea placeholder="Ex: Bomba de infusão, Ventilador, etc"
              value={equipamento}
              onChange={e=>setEquipamento(e.target.value)} required />
          </label>
          <label>
            Dúvida / instrução desejada 
            <textarea placeholder="Descreva qual instrução precisa"
              value={duvida}
              onChange={e=>setDuvida(e.target.value)} required />
          </label>
        </div>
